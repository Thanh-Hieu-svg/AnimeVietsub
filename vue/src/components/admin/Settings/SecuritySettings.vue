  ...
  <div class="bg-gradient-to-br from-[#1a1a1a] to-[#0f1416] border border-gray-800/50 rounded-xl p-6 shadow-xl animate-fade-in">
    ...
    <div class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label class="block text-sm font-semibold text-gray-400 mb-2">Số lần đăng nhập sai tối đa</label>
          <input
            :value="settings.maxLoginAttempts"
            @input="e => updateField('maxLoginAttempts', parseInt(e.target.value))"
            type="number"
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-400 mb-2">Thời gian khóa (phút)</label>
          <input
            :value="settings.lockoutDuration"
            @input="e => updateField('lockoutDuration', parseInt(e.target.value))"
            type="number"
            class="w-full bg-[#0f1416] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-[#b8e62e] focus:outline-none transition-all duration-300 focus:shadow-lg focus:shadow-[#b8e62e]/20"
          />
        </div>
        <div>
          <label class="block text-sm font-semibold text-gray-400 mb-2">Timeout phiên (phút)</label>
          <input
            :value="settings.sessionTimeout"
            @input="e => updateField('sessionTimeout', parseInt(e.target.value))"
            type="number"
            class="w-full bg-[#0f1416] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-[#b8e62e] focus:outline-none transition-all duration-300 focus:shadow-lg focus:shadow-[#b8e62e]/20"
          />
        </div>
      ...
      <div class="border-t border-gray-800 pt-4 space-y-3">
        <div class="flex items-center justify-between p-4 bg-black/20 rounded-lg hover:bg-black/30 transition-all">
          <div>
            <p class="text-white font-semibold">Bật Captcha</p>
            <p class="text-sm text-gray-400">Sử dụng Google reCAPTCHA</p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input 
              :checked="settings.enableCaptcha"
              @change="e => updateField('enableCaptcha', e.target.checked)"
              type="checkbox" 
              class="sr-only peer"
            >
        ...
        <div class="flex items-center justify-between p-4 bg-black/20 rounded-lg hover:bg-black/30 transition-all">
          <div>
            <p class="text-white font-semibold">Xác thực 2 yếu tố</p>
            <p class="text-sm text-gray-400">Bảo mật nâng cao cho admin</p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input 
              :checked="settings.enableTwoFactor"
              @change="e => updateField('enableTwoFactor', e.target.checked)"
              type="checkbox" 
              class="sr-only peer"
            >
            <div class="w-14 h-7 bg-gray-700 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#b8e62e]/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[4px] after:bg-white after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-[#b8e62e]"></div>
