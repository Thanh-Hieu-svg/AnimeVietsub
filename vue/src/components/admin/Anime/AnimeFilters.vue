...
const emit = defineEmits(['update:searchQuery', 'update:selectedStatus', 'update:selectedGenre', 'update:sortBy', 'bulk-action'])
</script>
...
<template>
  <div class="bg-gradient-to-br from-[#1a1a1a] to-[#0f1416] border border-gray-800/50 rounded-xl p-6 shadow-xl hover:border-[#b8e62e]/30 transition-all duration-300 animate-fade-in" style="animation-delay: 0.5s">
    ...
    <!-- Bulk Actions -->
    <div v-if="selectedAnimesCount > 0" class="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t border-gray-800/50 animate-slide-in-left">
      <p class="text-sm text-gray-400 font-semibold">{{ selectedAnimesCount }} anime được chọn</p>
      <button 
        @click="emit('bulk-action', 'featured')" 
        class="bg-[#b8e62e]/20 hover:bg-[#b8e62e]/30 text-[#b8e62e] px-4 py-2 rounded-lg font-semibold text-sm transition-all flex items-center gap-2 hover:scale-105 hover:shadow-lg hover:shadow-[#b8e62e]/50"
      >
        <font-awesome-icon icon="star" />
        <span>Nổi bật</span>
      </button>
      <button 
        @click="emit('bulk-action', 'hidden')" 
